...
url = 'https://forum.netmarble.com/api/game/stonemmogb/official/forum/stone_en/article/list?rows=15&start=0&menuSeq=7&_=1700000000000'

if __name__ == '__main__':
    newurl, aid, menuseq = serchurl(url, 'en')
    if newurl:
        # imgurl = serchimg(aid, menuseq)
        for i in range(8):
            try:
                sendmessage(userid[i], "更新资讯已发布，{}大佬请看:\n".format(username[i]) + newurl)
            except:
